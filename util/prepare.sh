...
if [ ! -d "qt" ]; then
    ...
    if [[ "$TRAVIS_OS_NAME" == "osx" ]]; then
        ...
        sed -i -e 's|MODULE|qt.qt5.'$QT_VERSION_JOINED'.clang_64|g' "$SELF_PATH/qt_install.qs"
        sudo ./run --script "$SELF_PATH/qt_install.qs" --no-force-installations
        rm run
        hdiutil unmount qt-installer
        ln -s /opt/qt/$QT_VERSION/clang_64 qt
    fi
    if [[ "$TRAVIS_OS_NAME" == "linux" ]]; then
        ...
        sed -i -e 's|INSTALL_PATH|/opt/qt|g' "$SELF_PATH/qt_install.qs"
        sed -i -e 's|MODULE|qt.qt5.'$QT_VERSION_JOINED'.gcc_64|g' "$SELF_PATH/qt_install.qs"
        sudo download/qt-installer.run --script "$SELF_PATH/qt_install.qs" --no-force-installations
        ln -s /opt/qt/$QT_VERSION/gcc_64 qt
    fi
...
#        sed -i -e 's|MODULE|qt.qt5.'$QT_VERSION_JOINED'.win64_mingw73|g' "$SELF_PATH/qt_install.qs"
        sed -i -e 's|MODULE|qt.qt5.'$QT_VERSION_JOINED'.win32_msvc2017|g' "$SELF_PATH/qt_install.qs"

        download/qt-installer.exe --script "$SELF_PATH/qt_install.qs" --no-force-installations
#        ln -s /c/Qt/$QT_VERSION/mingw73_64 qt
        ln -s /c/Qt/$QT_VERSION/msvc2017 qt
    fi
fi
